...
namespace Laundry
{
    ...
    public class LaundyMonitor
    {
        ...
        {
            Notify.MobileAppOneplusA6003(message: "clear_notification",
                                         data: new { tag = "WasherNotification" });
        }
        ...
        private void DryerReady()
        {
            Notify.MobileAppOneplusA6003(
                message: $"⌛ {TimeSpan.FromSeconds((long?)_entities.Sensor.DryerProgramTime.AsNumeric().State ?? 0)}" +
                          $"⚡ {_entities.Sensor.DryerProgramEnergy.AsNumeric().State:N0} {_entities.Sensor.DryerProgramEnergy.Attributes?.UnitOfMeasurement} " +
